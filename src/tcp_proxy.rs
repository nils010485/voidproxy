...
impl TcpProxy {
    ...
    async fn handle_connection_with_token(
        ...
        let server_stream = match TcpStream::connect(dst_addr).await {
            Ok(stream) => stream,
            Err(e) => {
                warn!(
                ...
                // Return early but count that we received a connection attempt
                let instances = instances.read().await;
                if let Some(instance) = instances.get(&instance_id) {
                    instance.metrics.add_bytes_received(1); // Count at least the connection attempt
                }
                return Ok(());
            }
        };
            ...
            tokio::spawn(async move {
                ...
                loop {
                    tokio::select! {
                        ...
                        read_result = reader.read_buf(buffer.as_mut()) => {
                            match read_result {
                                Ok(0) => break,
                                Ok(n) => {
